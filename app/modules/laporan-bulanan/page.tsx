...
export default function LapbulModulePage() {
  ...
  const ppatSummary = useMemo(() => {
    ...
    return { types, totalSSB, totalSSP };
  }, [ppatRecords]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
          ...
          <div className="lg:col-span-4 bg-white p-6 rounded-xl shadow-sm h-fit space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div><label className="block text-xs font-bold text-gray-500 mb-1">JUDUL / SIFAT AKTA</label><textarea rows={2} className="w-full border p-2 rounded" value={formState.judulAkta} onChange={e => setFormState({ ...formState, judulAkta: e.target.value })} placeholder="Jual Beli / Kuasa" /></div>
                ...
                <div key={idx} className={idx > 0 ? "break-before-page" : ""}>
                  <A4Container className="print-wrapper font-serif text-black">
                        ...
                        <tr>
                          ...
                          <td className="align-top py-0.5 pr-2 leading-tight">
                            <div className="flex">
                              ...
                              <span>Laporan Bulanan Pembuatan Akta oleh PPAT</span>
                            </div>
                          </td>
                          {/* Nama Penerima (Bold) */}
                    ...
                    <div className="text-[11pt] text-justify leading-relaxed">
                      <p className="mb-4">Dengan hormat,<br />Bersama dengan ini kami menyampaikan Laporan Bulanan Pembuatan Akta PPAT untuk Bulan <strong>{MONTHS[selectedMonth - 3].toUpperCase()} {selectedYear}</strong>, sesuai daftar terlampir dengan perincian sebagai berikut:</p>
                      <div className="p-2 mb-4">
                        <table className="w-full">
                          <tbody>
                            {Object.entries(ppatSummary.types).map(([k, v]) => <tr key={k}><td className="font-bold w-1/2">{k}</td><td>: {v} Akta</td></tr>)}
                            {Object.keys(ppatSummary.types).length === 0 && <tr><td className="text-center italic">NIHIL (Tidak ada akta)</td></tr>}
                            <tr><td colSpan={2} className="h-4"></td></tr>
                            <tr><td>Jumlah BPHTB (SSB)</td><td>: {currency(ppatSummary.totalSSB)}</td></tr>
                            <tr><td>Jumlah PPh (SSP)</td><td>: {currency(ppatSummary.totalSSP)}</td></tr>
                          </tbody>
                        </table>
                      </div>
                      <p>Demikian laporan ini saya sampaikan, atas perhatiannya saya ucapkan terima kasih.</p>
                    </div>
                    <div className="mt-12"><p>Hormat kami,</p><div className="h-20"></div><p className="font-bold underline">(HAVIS AKBAR, S.H., M.Kn.)</p></div>
                  </A4Container>
                  ...
                  <tbody>
                    {ppatRecords.length === 0 ? (
                      ...
                      ppatRecords.map((record, index) => {
                        // Simple heuristic to split parties based on role keywords
                        ...
                        // Fallback: if no roles match, put everyone in first column
                        const displayMengalihkan = mengalihkan || (!menerima ? record.pihak.map(p => p.name).join(', ') : '-');
                        const displayMenerima = menerima || '-';
                        ...
                        return (
                          <tr key={record.id} className="align-top">
                            ...
                            <td className="border border-gray-300 px-1 py-1">{displayMenerima}</td>
